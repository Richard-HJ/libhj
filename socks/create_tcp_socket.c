/*
       create_tcp_socket.c      R. Hughes-Jones  The University of Manchester
...
/* --------------------------------------------------------------------- */
{
  ...
  sock_create_tcp_socket( soc, soc_info, AF_INET);
}

void sock_create_tcp_socket( int *soc, SOC_INFO *soc_info, int ipfamily)
/* --------------------------------------------------------------------- */
{
  /* Open the TCP IP socket.  ( domain or family, socket type, protocol) */
  /* Local variables */
  int value;
  int def_value;
  int tos_value;
   ...
   *soc = socket(ipfamily , SOCK_STREAM, 0 ); 
   if (soc < 0) {
        perror("TCP IP socket failed to open :" );
        exit(-1);
    }
...
/* get the default TOS settings */
   ...
   if(!soc_info->quiet){
       printf("IP_TOS 0x%x IP prec %d IP tos %d ", def_value, ((def_value&IPTOS_PREC_MASK)>>5), ((def_value&IPTOS_TOS_MASK)>>1) );
   }

/* get the socket TCP/IP options */
    len = sizeof(def_value);
    def_value=0;
    error = getsockopt(*soc, IPPROTO_TCP, TCP_NODELAY, &def_value, (socklen_t *) &len);
    if(!soc_info->quiet){
        printf("TCP_NODELAY %d ", def_value );
	printf(" \n" );
    }
...
/* set the socket IP options tos and precidence_bits */
    if(soc_info->tos_set == 1){
       ...
       if (error) {
	   perror("setsockopt( IP_TOS) on socket failed :" );
	   exit(-1);
       }
    }
...
/* set the socket TCP options */
    len = sizeof(value);
    value = soc_info->enable_nodelay;
    error = setsockopt(*soc, IPPROTO_TCP, TCP_NODELAY, &value, len);

    len = sizeof(nodel_value);
    nodel_value=0;
    error = getsockopt(*soc, IPPROTO_TCP, TCP_NODELAY, &nodel_value, (socklen_t *) &len);

/* set socket options SO_REUSEADDR - so can re-listen if an old link is still in TIME_WAIT state*/
    len = sizeof(value);
    value = 1;
    error = setsockopt(*soc, SOL_SOCKET, SO_REUSEADDR, &value, len);
    if (error) {
...
/* print the values that were set */
	if(!soc_info->quiet){
        ...
        printf("TCP_NODELAY %d ", nodel_value );
		printf(" \n" );
	}
}
